...
startService() {
        ...
        babel-node --harmony main.js --debug-output=true --http.server.port=$2 --src-dirs=src --main-class=netease.smartwatch.backend.Main --config-files=config/smartwatch-backend-service.conf  --easynode.app.id=$1 > ../logs/$1.log 2>&1
}
